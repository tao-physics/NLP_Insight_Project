...
from model import LabelModel, LabelModel
...
def predict():
    ...
    sample['label_pred'] = model_prediction
    sample['label_2ndpred'] = model_2ndprediction
    sample['prob_1st'] = prob_1st
    sample['prob_2nd'] = prob_2nd
    for k in range(config.OUTPUT_SIZE):
        sample[f'prob_{k}rank'] = prob_all[k]
    sample.to_csv(config.OUTPUT_PATH + '/pred_all_prob_6aug.csv', index=False)
