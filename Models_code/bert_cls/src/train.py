...
def run(fold=0):
    ...
    model = SentimentModel(model_config, config.OUTPUT_SIZE_SENTIMENT)
    model.to(device)
    ...
    num_train_steps = int(len(df_train) / config.TEST_BATCH_SIZE * config.EPOCHS)
    ...
    for epoch in range(config.EPOCHS):
        ...
        early_stopping(losses_eval, model, 
                    model_path = config.OUTPUT_PATH + f'/model_{fold}.bin')
